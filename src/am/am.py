...
import torch
import torchhd
from torchhd import MCRTensor, CGRTensor
import logging
...
class AMCgr(BaseAM):
    ...
    def train_am(self):
        ...
        """
        # Convert from the inner complex representation to MCR
        self.am = CGRTensor.complex_to_mcr(self.weight, self.mod)
        am_complex = CGRTensor.mcr_to_complex(self.am, self.mod)
        self.am_complex = torchhd.ensure_vsa_tensor(am_complex, vsa='FHRR')
    ...
    def search(self, query: torch.Tensor):
        ...
        """
        #logit = torchhd.cosine_similarity(query, self.am)
        logit = CGRTensor.manhattan_similarity(query, self.am, self.mod)
    ...
    def add(self, input: torch.Tensor, idx: torch.Tensor, **kwargs):
        ...
        """
        #input = torchhd.hard_quantize(input)
        t = CGRTensor.mcr_to_complex(input, self.mod)
        self.weight[idx] = self.weight[idx] + t
